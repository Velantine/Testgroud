...
public class SpaceMovment : MonoBehaviour {
	...
	void Update () {
		...
		this.transform.rotation.eulerAngles.Set (this.transform.rotation.x * Input.GetAxis ("Mouse X")*speed, this.transform.rotation.y * Input.GetAxis ("Mouse Y")*speed, this.transform.rotation.z);
	}
}*/
using UnityEngine;
using System.Collections;
...
public class SpaceMovment : MonoBehaviour
{
	...
	void FixedUpdate()
	{
		...
		*/
		...
		//ship.transform.localPosition = cameraOffset + new Vector3(0, 0, -deltaSpeed * .02f);
		...
		transform.Translate((offsetDir * sqrOffset * speedM + ship.transform.forward * speed) * Time.fixedDeltaTime, Space.World);
