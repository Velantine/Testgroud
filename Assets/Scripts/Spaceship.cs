...
public class Spaceship : NetworkBehaviour {
	...
	private NetworkSpaceship spaceship;
	...
	void FixedUpdate()
	{
		if (isLocalPlayer)
		{
			...
			if (IsOverThreshold(lastSentPosition, pos, minPosDist) || IsOverThreshold(lastSentBodyRotation, bodyRot, minBodyRotDist))
			{
				CmdSendData(pos, bodyRot);
	...
	private void Interpolate()
	{
		...
		spaceship.CharacterController.Move(movement);
	...
	[Command]
	private void CmdSendData(Vector3 pos, float bodyRot)
	{
		syncPosition = pos;
